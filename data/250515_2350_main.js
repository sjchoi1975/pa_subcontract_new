    ...
    function updateGraph() {
        ...
        nodeGroupElementsSelection.select("circle")
            ...
            .style("fill", d => {
                ...
                return d.isPharmacy ? "crimson" : getContractorFillColor(d.childrenCount);
            })
            .style("fill-opacity", d => d.isPharmacy ? 1 : getContractorFillOpacity(d.childrenCount))
            .style("stroke", d => {
                if (window.selectedNodeId && d.id === window.selectedNodeId) return "rgb(30, 102, 161)";
                return d.isPharmacy ? "darkred" : getContractorStrokeColor(d.childrenCount);
            })
            .style("stroke-width", d => {
                if (window.selectedNodeId && d.id === window.selectedNodeId) return 3;
                return d.isPharmacy ? 2 : getContractorStrokeWidth(d.childrenCount);
            })
        ...
        nodeGroupElementsSelection.select("text")
            .style("fill", d => {
                if (window.selectedNodeId && d.id === window.selectedNodeId) return "rgb(30, 102, 161)";
                return d.isPharmacy ? "rgb(255,255,255)" : "rgb(0,0,0)";
            })
            .attr("dy", d => { 
                if (d.isPharmacy) {
