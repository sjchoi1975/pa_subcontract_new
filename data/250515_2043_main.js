document.addEventListener('DOMContentLoaded', async () => {
    ...
    function displayUserInfo(user, pharmacyDbInfo) {
        ...
        userSessionControls.innerHTML = `
            <div id="user-info" style="display: flex; align-items: center; justify-content: flex-end;">
    ...
    async function handleLogout() {
        ...
        if (error) {
            ...
            console.log('로그아웃 성공, login.html로 리디렉션합니다.');
    ...
    async function loadPrimaryContractors(pharmacyInfo) {
        ...
        initializeGraph(data || [], pharmacyInfo);
    }
    ...
    function updateGraph() {
        ...
        linkElementsSelection = linkElementsSelection.enter().append("line")
            ...
            .attr("marker-start", d => d.isBidirectional ? "url(#start)" : null)
            ...
            .on("click", async function(event, d) {
                ...
                function generateCompanyDetailsHtml(nodeData, childrenStatusMessage) {
                    ...
                    return `
                        ...
                        <p><strong>CSO 신고번호 :</strong> ${csoRegistNo}</p>
                        <p><strong>하위 업체 :</strong> ${childrenStatusMessage}</p>
                    `;
                }
                ...
                function generatePharmacyDetailsHtml(nodeData, childrenStatusMessage){
                    ...
                    const bizNo = nodeData.id || '번호 없음';
                    const ceo = nodeData.ceo || 'N/A'; // 대표자명 추가
                    const address = nodeData.address || 'N/A'; // 주소 추가
                    ...
                    return `
                        ...
                        <p><strong>사업자등록번호 :</strong> ${bizNo}</p>
                        <p><strong>주소 :</strong> ${address}</p>
                        <p><strong>1차 위탁업체 :</strong> ${childrenStatusMessage}</p>
                    `;
                }
        ...
        nodeGroupElementsSelection.select("circle")
            ...
            .style("opacity", 1)
            .style("fill", d => d.isPharmacy ? "crimson" : getContractorFillColor(d.childrenCount))
            .style("fill-opacity", d => d.isPharmacy ? 1 : getContractorFillOpacity(d.childrenCount))
            .style("stroke", d => d.isPharmacy ? "darkred" : getContractorStrokeColor(d.childrenCount))
            .style("stroke-width", d => d.isPharmacy ? 2 : getContractorStrokeWidth(d.childrenCount))
    ...
    function initializeGraph(initialContractorData, pharmacyData) {
        ...
        svgInstance.append("defs").selectAll("marker")
            .data(["end", "start"]) 
            .enter().append("marker")
            .attr("id", d => d) // marker id를 "end", "start"로 수정
